  ...
  configVxeTable: (vxeUI) => {
    ...
    vxeUI.renderer.add('CellDict', {
      renderTableDefault(renderOpts, params) {
        ...
        const cellVal = row[column.field];
        // 如果值是数组，或者显式声明 multiple，则渲染多个标签
        if (Array.isArray(cellVal) || props.multiple) {
          const values = Array.isArray(cellVal) ? cellVal : [cellVal];
          return h(
            'div',
            { class: 'flex flex-wrap gap-1' },
            values.map((v: any) =>
              h(DictTag, { type: props.type, value: v?.toString() }),
            ),
          );
        }
        // 单值渲染
