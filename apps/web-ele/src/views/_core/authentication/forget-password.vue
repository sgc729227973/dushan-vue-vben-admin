...
import { getDefaultCaptchaType, isTenantEnable } from '@vben/hooks';
...
const captchaEnable = true; // 强制启用验证码，保护短信验证码接口
...
const formSchema = computed((): VbenFormSchema[] => {
  return [
    ...
    },
    {
      ...
      rules: z
        .string()
        .nonempty({ message: $t('authentication.mobileTip') })
...
async function sendSmsVerificationCode(
  ...
  try {
    ...
    ElMessage.success($t('authentication.sendCodeSuccess'));
    return true;
  } catch (error) {
