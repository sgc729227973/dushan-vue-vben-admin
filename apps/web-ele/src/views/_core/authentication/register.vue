...
import { AuthenticationRegister, UniversalCaptcha, z } from '@vben/common-ui';
import {
  getClientId,
...
async function handleRegister(values: any) {
  ...
  if (!values.inviteCode) {
    ElMessage.error($t('authentication.inviteCodeRequired'));
    return;
  }
...
/** 验证码通过，执行注册 */
const handleCaptchaSuccess = async ({ captchaVerification }: any) => {
  try {
...
const formSchema = computed((): VbenFormSchema[] => {
  return [
    ...
    },
    {
      component: 'VbenInput',
      componentProps: {
        placeholder: $t('authentication.inviteCodeTip'),
      },
      fieldName: 'inviteCode',
      label: $t('authentication.inviteCode'),
      rules: z
        .string()
        .min(1, { message: $t('authentication.inviteCodeRequired') }),
    },
    {
      component: 'VbenInput',
      componentProps: {
